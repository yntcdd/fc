...
import math
import random
from deepseek_ai import create_ai, _best_pass, _path_blocked, _dist
import claude_ai  # registers "claude_gk", "claude_pm", "claude_def", "claude_str"
...
try:
    _z_img = pygame.image.load("images/z.jpg").convert_alpha()
    _z_img = pygame.transform.smoothscale(_z_img, (_IMG_SIZE, _IMG_SIZE))
    _z_img = _make_circular(_z_img)
except Exception:
    _z_img = _deepseek_img
...
# deepseek AIs: "deepseek_gk" "deepseek_pm" "deepseek_str" "deepseek_def" "deepseek_trickster"
# claude AIs:   "claude_gk" "claude_pm" "claude_def" "claude_str"
...
#   PLAYER3_AI = create_ai("deepseek_str")
#   PLAYER7_AI = create_ai("deepseek_trickster")
#   PLAYER1_AI = create_ai("deepseek_gk")
...
# AI players must shoot or pass the ball after holding it this long
BALL_HOLD_LIMIT = 90   # 1.5 seconds at 60 fps
...
class Player:
    def __init__(self, x, y, color, keys, kick_key, display_name=""):
        ...
        self.hold_timer = 0      # frames the player has held the ball (AI forced-release)
        self.no_pickup_timer = 0 # frames the player cannot claim the ball (forced-release recoil)
            ...
            if distance < self.radius + ball_radius:
                ...
                if (last_kicker is not None and
                    ...
                    is_gk = (hasattr(last_kicker, 'ai') and last_kicker.ai is not None
                             and last_kicker.ai.name in ('Deepseek GK', 'Claude GK', 'Z GK'))
                    if not is_gk:
...
def force_ai_release(player, teammates, opponents, attacking_right):
    """Force an AI ball-carrier to get rid of the ball right now:
    shoot at the opponent goal when a lane is open, otherwise pass to
    the best teammate (or clear upfield if nobody is available)."""
    goal_x = WIDTH - GOAL_WIDTH if attacking_right else GOAL_WIDTH
    goal_y = HEIGHT // 2

    # Prefer shooting at the far post, then the near post
    far_y = goal_y + (GOAL_HEIGHT // 2 - 25 if player.y <= goal_y
                      else -(GOAL_HEIGHT // 2 - 25))
    tx = ty = None
    power = 0
    for corner_y in (far_y, goal_y - (far_y - goal_y)):
        if not _path_blocked(player.x, player.y, goal_x, corner_y, opponents, 40):
            tx, ty, power = goal_x, corner_y, MAX_KICK_POWER
            break

    if tx is None:
        # Shot lanes are covered — pass to the best teammate instead
        mate, _ = _best_pass(player, teammates, opponents, attacking_right)
        # Avoid recycling the ball straight back to our own goalkeeper
        if mate is not None and (mate is player1 or mate is player2):
            non_gk = [t for t in teammates
                      if t is not player and not t.holding_ball
                      and t is not player1 and t is not player2]
            mate = (min(non_gk, key=lambda t: _dist(player.x, player.y, t.x, t.y))
                    if non_gk else None)
        if mate is not None:
            tx, ty, power = mate.x, mate.y, 11   # firm pass
        else:
            # Nobody to pass to — clear the ball toward the opponent goal
            tx, ty, power = goal_x, goal_y, MAX_KICK_POWER
    ...
    player.face_x = dx / dist
    player.face_y = dy / dist
    player.kick_power = power
    # Fire the kick immediately by releasing with no key held
    player.release_kick({player.kick_key: False})
    # Recoil: the player can't instantly re-claim the ball they were
    # forced to release (about a third of a second).
    player.no_pickup_timer = 20
    ...
    if game_state == "playing" and game_timer > 0:
        ...
        for p in all_players:
            ...
            if p.ai is not None:
                ...
                # Forced release: AI players must shoot or pass the ball
                # after holding it for BALL_HOLD_LIMIT frames (1.5 s).
                if p.holding_ball:
                    p.hold_timer += 1
                    if p.hold_timer >= BALL_HOLD_LIMIT:
                        force_ai_release(p, teammates, opponents, attacking_right)
                        p.hold_timer = 0   # fresh window if they regain the ball
                else:
                    p.hold_timer = 0
            else:
                # keyboard controlled
        ...
        if not any(p.holding_ball for p in all_players):
            ...
            candidates = [p for p in all_players
                          if p.no_pickup_timer <= 0
                          and (p.stunned == 0 or (hasattr(p, 'ai') and p.ai is not None
                                                  and p.ai.name in ('Deepseek GK', 'Claude GK', 'Z GK')))]
    ...
    if game_state == "replay" and replay_index < len(replay_buffer):
        ...
        for pd in frame["players"]:
            ...
            if img_type == "gemini":
                ...
                screen.blit(_z_img, (int(pd["x"] - _IMG_SIZE // 2), int(pd["y"] - _IMG_SIZE // 2)))
